...
from neutron.db.models.plugins.ml2 import qinqallocation as qinq_alloc_model
...
_deprecate._moved_global('QinqAllocation', new_module=qinq_alloc_model)

driver_type.register_ml2_drivers_qinq_opts()
...
class QinqTypeDriver(helpers.SegmentTypeDriver):
    ...
    def __init__(self):
        print('init qinq driver')
        config.cfg.CONF.set_override('network_qinq_ranges', ['1:300'], group='ml2_type_qinq')
        super(QinqTypeDriver, self).__init__(qinq_alloc_model.QinqAllocation)
    ...
    def _parse_network_qinq_ranges(self):
        ...
        try:
            for entry in cfg.CONF.ml2_type_qinq.network_qinq_ranges:
                ...
                print isinstance(range(int(qinq_min), int(qinq_max)), list)
    ...
    def release_segment(self, session, segment):
        ...
        with session.begin(subtransactions=True):
            query = (session.query(qinq_alloc_model.QinqAllocation).
    ...
    def allocate_partially_specified_segment(self, session, **filters):
        ...
        with session.begin(subtransactions=True):
            ...
            alloc = qinq_alloc_model.QinqAllocation(
